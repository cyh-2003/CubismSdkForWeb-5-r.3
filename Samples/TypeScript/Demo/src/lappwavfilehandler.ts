...
/** @deprecated 此变量由于 getInstance() 已被弃用，因此也被弃用 */
...
export class LAppWavFileHandler {
   ...
   *
   * @return 类的实例
   * @deprecated 该类中的单例模式已弃用。请改用 new LAppWavFileHandler()
   */
...
export class WavFileInfo {
  ...
  _bitsPerSample: number; ///< 每个样本的比特数
...
export class ByteReader {
  ...
  /**
   * @brief 读取24位（小端序）
   * @return Csm::csmUint32 读取的24位值（设置在下24位）
   */
  ...
  _fileByte: ArrayBuffer; ///< 加载文件的字节数组
